...
var tagsCount: Int {
    ...
    request.predicate = NSPredicate(format: "ANY items == %@", self)  // inverse 来自 @RelationshipInfo
...
@PersistentModel(
    ...
    relationshipCountPolicy: .none         // [待议] *Count：plain / warning / none（需 @RelationshipInfo）
)
```
...
- `relationshipCountPolicy`：控制 `*Count` 属性（此能力是否实现待议）；若实现，默认 `.none`，且仅在存在 `@RelationshipInfo` 时允许生成
...
属性可以通过 `@Attribute(original:)` 映射底层字段名，但**关系不提供此能力**，Swift 侧的属性名必须与 xcdatamodeld 中的关系名保持一致。
...
| 底层名映射 | `@Attribute(original:)` | 不支持 |
| 改名方式 | 代码层映射 | xcdatamodeld Renaming ID |
| 宏是否需要标注 | 是（`original:`）| 否 |
...
### @RelationshipInfo：描述性注释宏

为了让代码本身能传达关系的语义（无需翻 xcdatamodeld），提供一个**纯描述性**的注释宏。它不生成任何代码，对运行时零影响，本质上是结构化的代码注释，由工具自动生成。
...
宏实现为 `@attached(peer)` 展开为空，编译期完全透明。`@RelationshipInfo` 是工具与开发者之间的**契约**，而非运行时指令。
...
`@RelationshipInfo` 是可选的，工具根据是否存在来决定是否校验：

- **有 `@RelationshipInfo`**：工具校验宏参数与 xcdatamodeld 中的实际配置是否一致，不一致则报错
- **无 `@RelationshipInfo`**：默认模式下跳过，输出 `[INFO]` 提示；`--strict` 模式下视为警告

无论是否存在 `@RelationshipInfo`，工具都会强制检查模型层 inverse 是否配置；缺失 inverse 一律为 `[ERROR]`。
关系元信息依赖的生成项（如 `*Count`）仅在存在 `@RelationshipInfo` 时生成；即使主宏策略允许，缺少 `@RelationshipInfo` 也不会生成。

```
// 默认模式
[INFO]  Item.tags: no @RelationshipInfo, skipping validation

// --strict 模式
[WARN]  Item.tags: missing @RelationshipInfo annotation
```

这样新项目可以从一开始就开启 `--strict` 全面覆盖，旧项目迁移时用默认模式逐步补全，工具的生成命令也可以自动补上缺失的 `@RelationshipInfo`。
...
public final class Item: NSManagedObject {
    ...
    // [待议] 仅当 @RelationshipInfo 存在且 relationshipCountPolicy != .none 时生成
...
- Swift 对外属性名与持久化字段名不一致（`@Attribute(original:)`）
...
- 提供全局错误处理策略（供 `.codable` / `.transformed` 展开代码复用）
- 提供关系代码生成策略（getter/setter/count；其中 setter 仅 `Set<T>` 生效，count 需 `@RelationshipInfo`）
...
`@PersistentModel` 允许统一配置非基础类型存取失败时的处理策略：

```swift
@PersistentModel(serializationErrorPolicy: .logAndNil)
final class Item: NSManagedObject { ... }
```
...
enum SerializationErrorPolicy {
    case silentNil    // 默认：吞错并返回 nil（最保守）
    case logAndNil    // 记录日志并返回 nil
    case assertion    // Debug 下 assert，Release 返回 nil
}
```

`.codable` 与 `.transformed` 的宏展开代码统一走该策略，避免在每个属性上重复定义错误处理逻辑。
...
enum StorageMethod {
    ...
    case transformed(NSValueTransformer.Type)  // NSValueTransformer（传统 ObjC 转换）
...
```swift
@Attribute(original: "status", storageMethod: .raw)
...
```swift
@Attribute(original: "metadata", storageMethod: .codable)
...
var metadata: MyConfig? {
    get {
        ...
        do {
            ...
            // 按 @PersistentModel(serializationErrorPolicy:) 处理
            return nil
        }
    }
    set {
...
### .transformed — NSValueTransformer

适用于迁移已有 `NSValueTransformer` 子类的场景：

```swift
@Attribute(original: "color", storageMethod: .transformed(ColorTransformer.self))
var color: UIColor?

// 展开：
var color: UIColor? {
    get {
        // 失败时按 @PersistentModel(serializationErrorPolicy:) 处理
...
注意：需要在 xcdatamodeld 中将该属性设为 `Transformable` 并指定 transformer 名称。
...
|---|---|
| `String` / `Bool` / `Data` / `Date` / `UUID` / `Int` / `Float` / `Double`（含可选） | `.default` |
...
| 持久化字段名 | `@Attribute(original:)` 中的名称是否在模型中存在 |
...
| `@RelationshipInfo` 一致性 | 若存在，校验 `inverse`、`deleteRule`、`ordered` 与模型是否一致 |
...
final class Item: NSManagedObject, Identifiable {
    ...
    @Attribute(original: "status")
    var status: String?

    // TODO: rename if needed. persistent key: "count"
    @Attribute(original: "count")
    var count: Int?
}
```
