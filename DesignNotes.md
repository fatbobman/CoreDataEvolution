...
- 无法唯一推断 inverse 的 relationship，两侧都必须显式标注 `@Inverse(...)`。这是一条 runtime/test-debug 路径与 tooling validate 共用的源码规则，不影响基于 `xcdatamodeld` 的正式模型工作流。
...
- 若当前实体内有多条 relationship 指向同一目标实体，则这些 relationship 两侧都必须显式标注 `@Inverse(...)`
