...
struct GeneratedFlowApp {
  static func main() throws {
    ...
    let lines = try fetch(in: container.viewContext)
    for line in lines {
      print(line)
    }
  }
  ...
  private static func fetch(in context: NSManagedObjectContext) throws -> [String] {
    ...
    return try context.fetch(request).map { task in
